...
// A tiny dependency-free seeded PRNG: xmur3 string-hash → mulberry32 stream. Same
// seed always yields the same [0,1) sequence — used to derive a stable package from
// a candidate's id, so what the selection card shows is exactly what marriage grants.
...
// The exact package a given candidate brings, derived deterministically from her id
// (stable across reads) and always within the ruled ranges — the same value the
// selection card shows and marry() grants.
export function bridePackageForCandidate(candidateId: string): { slaves: number; wool: number; oliveoil: number } {
  return rollBridePackage(seededRng(candidateId));
}
