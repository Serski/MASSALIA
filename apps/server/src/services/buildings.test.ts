...
suite("Ledger / building engine (integration)", () => {
  ...
  it("day-1 path: 100dr → build Estate T1 (25) → wallet 75 → constructs in 1 hour → first collect yields grain", async () => {
    const c = await ctx();
    const built = await m.buildings.build("landowner", c, "estate", new Date(T0));
    expect(built.ok).toBe(true);
    if (built.ok) expect(built.cost).toBe(25);
    expect(await wallet()).toBe(75);
  ...
  it("(P3a) build is rejected — and nothing mutated — when drachmae, materials, OR pops are short", async () => {
    ...
    // (i) Insufficient drachmae: estate T1 costs 25; give 10. Materials + slaves are present.
    playerId = await freshPlayer(10, "landowner");
    let c = await ctx();
    ...
    expect(await wallet()).toBe(10); // not debited
  ...
  it("(P3b) happy build debits drachmae + exactly the right materials, creates the building, consumes NO pops", async () => {
    playerId = await freshPlayer(100, "landowner"); // estate T1: 25dr, {timber:8, stone:6}, needs 2 slaves owned
    const c = await ctx();
